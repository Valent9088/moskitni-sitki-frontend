...
export const CATEGORIES = {
  ...
  windowRoller: {
    ...
    types: {
      normal: {
        name: "Звичайна", // Прибрано "(без нижньої планки)"
  ...
  doorPliseStandard: {
    ...
    sMin: 0.9,
    canvases: { black: 0 },
    colors: { white: 4450, brown: 4600 }, // Видалено Антрацит
  },
};
...
export function describeSelection(selection) {
  ...
  if (selection.modelKey && category.models) {
    parts.push(category.models[selection.modelKey]?.name || selection.modelKey);
  }
  if (selection.typeKey && category.types) {
    parts.push(category.types[selection.typeKey]?.name || selection.typeKey);
  }
  if (selection.colorKey) {
    parts.push(COLOR_LABELS[selection.colorKey] || selection.colorKey);
  }
  if (selection.canvasKey) {
    parts.push(CANVAS_LABELS[selection.canvasKey] || selection.canvasKey);
  }
